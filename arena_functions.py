...
def save_debug_screenshot(name: str, coords: Vec4) -> None:
    """保存带红框标记的调试截图"""
    # 截取全屏
    screen_capture = ImageGrab.grab(bbox=(0, 0, 1920, 1080))
    ...
    # 绘制标尺
    scale_start_x = coords.get_coords()[0] 
    scale_start_y = coords.get_coords()[3]
    
    # 绘制水平标尺
    draw.line(
        (scale_start_x, scale_start_y, scale_start_x + 50, scale_start_y),
        fill="red",
        width=2
    )
    ...
    # 绘制垂直标尺
    draw.line(
        (scale_start_x, scale_start_y, scale_start_x, scale_start_y - 50),
        fill="red", 
        width=2
    )
...
def get_abnormal() -> str:
    ...
    try:
        ...
        save_debug_screenshot("S13异常突变buff", screen_coords.ABNORMAL_POS)
        return ""
